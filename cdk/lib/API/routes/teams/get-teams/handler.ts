...
export async function handler(
  ...
  // Handler Logic
  // TODO: query tenant memberships and ensure that team has access to at least one tenant that requestor has access to
  try {
    const type = "team";
    ...
    // This is insecure
    const result = teamId
      ? await Team.query.byTeam({ id: teamId, type }).go({ table })
      : await Team.query.all({ ...queryParams }).go({ table });
